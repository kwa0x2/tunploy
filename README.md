...
When a new release is out, the panel offers **Update now** and updates itself, rolling back on its own if the new version doesn't start. The install also adds a `tunploy` command on the server for resetting the admin password, restoring backups, reading logs and removing Tunploy (`tunploy uninstall`); run `tunploy help`.
